...
public final class Strings {
     ...
     *
     * @param str    the string to me converted.
     * @param prefix the string that precedes the result.
        ...
        while (matcher.find()) {
            for (int i = 1; i <= matcher.groupCount(); i++) {
                if (!matcher.group(i).isEmpty()) {
                    sb.append(matcher.group(i).substring(0, 1).toUpperCase()).append(matcher.group(i).substring(1).toLowerCase());
                }
            }
        }
